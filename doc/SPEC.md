...
2. **ドキュメント更新**
   - `doc/PR/PRxx.md`もしくは`doc/ISSUExx.md`ファイルを作成（xxはPR番号）
...
### Phase 5.5: デプロイと公開（PR#9完了後）

#### PR#10: Vercelへのデプロイ

**目的**: フロントエンドアプリを本番環境に公開し、スマホでの動作確認を可能にする

**タスク**:

- [ ] **Vercelアカウントの作成**
  - https://vercel.com にアクセス
  - GitHubアカウントを使用してサインアップ

- [ ] **プロジェクトのデプロイ**
  - Dashboard → "Add New" → "Project"
  - GitHubリポジトリ `line_talk_analyzer_frontend` を接続
  - Root Directoryを `.` に設定
...
- [ ] **環境変数の設定**
  ...
  ※ バックエンドのRender URLを設定

- [ ] **デプロイの実行**
  - "Deploy" をクリック
  - 自動的にビルド＆デプロイが開始
  - URLが発行される（例: `https://line-talk-analyzer.vercel.app`）

- [ ] **バックエンドCORS設定の更新**
  - バックエンドの環境変数 `ALLOWED_ORIGINS` にVercel URLを追加
  - Renderダッシュボードで環境変数を更新:
    ```
    ALLOWED_ORIGINS=http://localhost:3000,https://line-talk-analyzer.vercel.app
    ```
  - サービスを再デプロイ
...
- [ ] **動作確認**
  - デスクトップブラウザでアクセス
  - スマホブラウザでアクセス
  - ファイルアップロード → 解析 → 結果表示の一連の流れを確認
  - 初回アクセス時のバックエンド起動待ち（30秒程度）を確認

- [ ] **ドメイン設定（オプション）**
  - Vercel Dashboardで "Domains" タブ
  - カスタムドメインの追加（お名前.comなどで取得）
  - DNS設定の更新

- [ ] **README.mdの更新**
  - デプロイ済みURLの記載
  - 使い方の説明（スマホでのアクセス方法）
  - 初回アクセス時の注意事項（バックエンド起動待ち）
...
- [ ] パフォーマンステスト
  - Lighthouseスコアの確認（Performance, Accessibility, Best Practices, SEO）
  - 目標: Performance 80以上

- [ ] ネットワークテスト
  - バックエンドのスリープ状態からの復帰確認
  - エラーハンドリングの確認

**依存**: PR#9（結果表示ページ）、バックエンドPR#10（Renderデプロイ）

---

#### PR#11: アニメーション演出（オプション）
...
**依存**: PR#10（Vercelデプロイ）、PR#11（アニメーション演出、オプション）
...
**依存**: PR#12（E2Eテスト）、PR#11（アニメーション演出、オプション）

---

### Issue対応

#### Issue#01: 重いファイルを渡したときにタイムアウトする問題の修正

**目的**: 大容量ファイルアップロード時のタイムアウト問題を解決

**背景**:
- LINEのトーク履歴保存時に期間指定ができないため、長期間の会話履歴が全て含まれる
- 解析対象は今年の会話のみだが、何年分ものデータがサーバーに送信される
- サーバー側の処理時間が増大し、タイムアウトが発生する

**解決方針**:
- フロント側でファイル読み込み後、対象期間のメッセージのみを抽出
- 抽出後のデータのみをサーバーに送信することで、データ量と処理時間を削減
- サーバー側の期間フィルタリング処理は二重チェックとして維持

**タスク**:

1. **ファイル解析・抽出機能の実装**
   - [x] `src/lib/fileUtils.ts` の作成
     - [x] `extractDateRange(content: string, startDate: Date, endDate: Date): string` - 行レベルでの高速抽出（推奨実装）
       - **最適化アプローチ**: LINEトーク履歴が時系列順に保存されている特性を利用
       - ファイル先頭のヘッダー部分を保持（3行程度: タイトル、保存日時、空行）
       - 日付行（`YYYY/MM/DD(曜日)` 形式）を検出しながら行単位で処理
       - 開始日より前の日付が現れるまでの行はスキップ
       - 開始日〜終了日の範囲の行を抽出
       - 終了日より後の日付が現れたら以降の行は破棄
       - **利点**: 全メッセージのパース不要、メモリ効率良好、大容量ファイルで高速
     - [x] `parseLineTalkHistory(content: string): Message[]` - LINEトーク履歴のパース処理（フォールバック/デバッグ用）
       - 日付行、発言者行、メッセージ本文の識別
       - バックエンドの `parser.py` を参考にロジック実装
       - 日付フォーマットの正規化（YYYY/MM/DD, YYYY/M/D等の対応）
     - [x] `filterMessagesByDateRange(messages: Message[], startDate: Date, endDate: Date): Message[]` - 期間フィルタリング（parseLineTalkHistory使用時）
       - タイムゾーン考慮（JST想定）
       - 境界値の扱い（開始日の0:00:00、終了日の23:59:59まで含む）
   - [x] 型定義 `src/types/file.ts` の作成
     ```typescript
     interface Message {
       date: Date;
       time: string;
...
2. **UI/UX改善**
   - [x] 抽出処理中のローディング表示
     - [ ] プログレスバー（大容量ファイル対応）
     - [x] 処理ステータス表示（"期間を抽出しています..."）

3. **解析フローの変更**
   - [x] `useAnalyze.ts` の修正
     - [x] ファイルアップロード → 期間抽出 → API送信の流れに変更
     - [x] 抽出処理のエラーハンドリング追加
   - [ ] Web Worker検討（オプション）
     - [ ] 大容量ファイルの処理をメインスレッドから分離
     - [ ] UIのフリーズ防止
...
4. **エラーハンドリング**
   - [x] パースエラーの処理
     ...
     - [ ] リトライ機能の提供
   - [x] 空結果の処理
     - [x] 該当期間にメッセージがない場合の警告

**テスト計画**:

- [x] 単体テスト: `tests/unit/lib/fileUtils.test.ts`
  - [x] `extractDateRange`: 正常なフォーマットでの期間抽出
  - [x] `extractDateRange`: ヘッダー部分の保持（タイトル、保存日時）
  - [x] `extractDateRange`: 開始日より前のデータがスキップされること
  - [x] `extractDateRange`: 終了日より後のデータが含まれないこと
  - [x] `extractDateRange`: 境界値の正確な処理（開始日当日、終了日当日を含む）
  - [x] `extractDateRange`: 複数の日付フォーマット対応（YYYY/MM/DD, YYYY/M/D）
  - [x] `extractDateRange`: 該当期間にデータがない場合の処理（ヘッダーのみ返却）
  - [ ] `extractDateRange`: 不正なフォーマットでのエラーハンドリング
  - [x] `parseLineTalkHistory`: 正常なフォーマットのパース（参考実装）
  - [x] `filterMessagesByDateRange`: 期間内のメッセージのみ抽出（参考実装）

- [x] 統合テスト: `tests/integration/fileExtraction.test.ts`
  - [x] ファイル選択 → 抽出 → API送信の一連のフロー
  - [x] 抽出前後のメッセージ数表示
  - [x] エラー時のユーザーフィードバック

- [ ] パフォーマンステスト
  - [ ] 大容量ファイル（10MB以上、数年分）の処理時間測定
  - [ ] extractDateRange方式 vs 全パース方式の速度比較
  - [ ] メモリ使用量の確認
  - [ ] UIのレスポンシブ性確認

**実装時の注意点**:

- **推奨実装**: `extractDateRange` による行レベル抽出を優先実装すること
  - LINEトーク履歴の時系列順特性を活用した最適化手法
  - ヘッダー（`[LINE] xxx のトーク履歴` 等）は必ず保持すること（先頭3行程度）
  - 日付行の正規表現: `/^\d{4}\/\d{1,2}\/\d{1,2}\(.+\)$/` を使用
  - 開始日より前: スキップ / 範囲内: 蓄積 / 終了日より後: 終了
- バックエンドの `parser.py` と同じ日付解析ロジックを使用し、整合性を保つこと
- `talk/sample.txt` を参照してLINEトーク履歴のフォーマットを確認すること
- バックエンド側の期間フィルタリングは削除せず、二重チェックとして残すこと
- ブラウザのメモリ制限を考慮すること（`extractDateRange`方式なら問題なし）
- 処理の進捗状況をユーザーに適切にフィードバックすること
- パフォーマンステストで最適化効果を測定・検証すること

**依存**: PR#8（API連携・解析処理）

**優先度**: 高（ユーザビリティに直結する問題）
