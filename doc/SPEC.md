...
- [ ] `src/hooks/useServerWarmup.ts` の実装
  - ヘルスチェックを非同期で送信する関数
...
- [ ] `src/hooks/useFileUpload.ts` の修正
  - ファイルアップロード成功時にウォームアップを実行
  - エラーハンドリング（ウォームアップ失敗時も処理を継続）

- [ ] `src/components/upload/PrivacyConsent.tsx` の修正
...
- 単体テスト: `tests/unit/hooks/`
  - [ ] useServerWarmup: 初回呼び出しでヘルスチェック実行
  - [ ] useServerWarmup: 2回目以降の呼び出しでスキップ
  - [ ] useServerWarmup: エラー時も例外をスローしない

- 統合テスト: `tests/integration/`
  - [ ] ファイルアップロード時にウォームアップが実行されること
  - [ ] プライバシーポリシー表示時にウォームアップが実行されること
  - [ ] ウォームアップ失敗時も解析処理が正常に動作すること
  - [ ] 重複リクエストが発生しないこと

- E2Eテスト: `tests/e2e/`
  - [ ] サーバーがスリープ状態からでも解析が完了すること
  - [ ] ウォームアップ後の解析処理が高速に完了すること（体感チェック）
